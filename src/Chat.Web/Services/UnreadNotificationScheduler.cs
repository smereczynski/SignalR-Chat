            ...
            try
            {
                ...
                foreach (var kv in emailDests)
                {
                    try
                    {
                        ...
                        _logger.LogInformation("Unread notification (email) queued for {Dest} (users: {Users})", kv.Key, string.Join(',', kv.Value));
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "Failed to send unread email notification to {Dest}", kv.Key);
                    }
                }
                ...
                foreach (var kv in smsDests)
                {
                    ...
                    {
                        await _otpSender.SendAsync(string.Join(',', kv.Value), kv.Key, body).ConfigureAwait(false);
                        _logger.LogInformation("Unread notification (sms) queued for {Dest} (users: {Users})", kv.Key, string.Join(',', kv.Value));
                    }
                    catch (Exception ex)
                    {
                        _logger.LogWarning(ex, "Failed to send unread sms notification to {Dest}", kv.Key);
                    }
                }
