...
1. **Cosmos DB (NoSQL API)** – Durable storage for messages, users, rooms, dispatch centers.
...
### 3.4 Container: `dispatchcenters`
| Aspect | Value |
|--------|-------|
| Partition Key | `/id` |
| Document ID | GUID/string identifier generated by API/repository |
| Rationale | Stable key-based CRUD and relationship validation (`correspondingDispatchCenterIds`) |

**Domain invariants (enforced in create flows):**
- At most one dispatch office with `ifMain = true` per `country` (case-insensitive country comparison).
- `correspondingDispatchCenterIds` disallow self-reference and are validated against existing dispatch center IDs.

### 3.5 Item Shape Examples
...
// users container document
{
  ...
  "dispatchCenterIds": ["dc-ops", "dc-pl-main"] // Optional list of dispatch center IDs
}
...
// dispatchcenters container document
{
  "id": "dc-pl-main",                          // Partition key + identifier
  "name": "Poland Main Dispatch",
  "country": "PL",
  "ifMain": true,
  "correspondingDispatchCenterIds": ["dc-de-berlin", "dc-fr-paris"],
  "users": ["alice", "bob"]                 // Denormalized assigned usernames
}
```
...
| `IDispatchCentersRepository` | `GetAllAsync`, `GetByIdAsync`, `GetByNameAsync`, `UpsertAsync`, `DeleteAsync`, `AssignUserAsync`, `UnassignUserAsync` | Organizational structure storage and user assignment management. |

**Admin UI behavior (`/Admin/DispatchCenters`):**
- Related dispatch offices are rendered as `Name (Country)` rather than raw IDs.
- Create page enforces the one-main-per-country invariant and blocks invalid submissions.
...
| Dispatch center seeding | Dispatch centers are **not** auto-seeded during deployment/startup; create via admin/API flows. |
| Main per country rule | Exactly one `ifMain=true` dispatch office per country is allowed during creation flows (admin UI + API). |
...
- `src/Chat.Web/Repositories/IDispatchCentersRepository.cs`
- `src/Chat.Web/Controllers/DispatchCentersController.cs`
